...
(() => {
  ...
  class StemMixer {
    constructor() {
      ...
      // continuous system hum (web-audio noise) — not a short looping sfx
      this.hum = { enabled: false, gain: null, src: null, filter: null };
    ...
    _initHum() {
      try {
        ...
        // Make a long-ish noise buffer to avoid obvious looping, then filter it.
        const len = Math.floor(this.ctx.sampleRate * 12); // 12s loop, heavily filtered
    ...
    setHum(on) {
      ...
      try {
        ...
        this.hum.gain.gain.setTargetAtTime(target, now, 0.15);
      } catch {}
    }
